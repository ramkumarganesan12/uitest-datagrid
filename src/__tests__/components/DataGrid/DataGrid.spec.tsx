...
describe("Data Grid", () => {
    ...
    describe("Row Selection", () => {
        ...
        it("should show checked state when all rows are selected", () => {
            const checkbox1 = screen.getByRole("checkbox", { name: "A380" });
            const checkbox2 = screen.getByRole("checkbox", { name: "A350" });
            const checkbox3 = screen.getByRole("checkbox", { name: "B787" });
    ...
    describe("Select All Actions", () => {
        ...
        it("should select all the rows on checking select all and deselect all rows on unchecking", () => {
            ...
            expect(screen.getByRole("checkbox", { name: "A380" })).toBeChecked();
            expect(screen.getByRole("checkbox", { name: "A350" })).toBeChecked();
            expect(screen.getByRole("checkbox", { name: "B787" })).toBeChecked();

            fireEvent.click(selectAllCheckbox);

            expect(screen.getByRole("checkbox", { name: "A380" })).not.toBeChecked();
            expect(screen.getByRole("checkbox", { name: "A350" })).not.toBeChecked();
            expect(screen.getByRole("checkbox", { name: "B787" })).not.toBeChecked();
    ...
    describe("Table Actions", () => {
        ...
        it("should render the button and on click calls the action function with selected data", () => {
            ...
            const actionButton = screen.getByRole("button", { name: "Extract" });
            const checkbox1 = screen.getByRole("checkbox", { name: "A380" });
