...
export const DataGrid = <T extends Record<string, string | number>>(
    ...
    if (selectedSet.length > 0 && selectedSet.length < data.length) {
        selectAllState = "mixed";
    } else if (selectedSet.length === data.length) {
    ...
    return (
        <div className={`${styles.dataGrid} ${darkmode ? styles.darkMode : ''}`}>
            ...
            {rowSelection && <div className={styles.dataToolBar}>
                ...
                <tbody>
                    {
                        data.map((rowData, index) => (
                            <DataRow
                                ...
                                selected={selectedSet.filter((row) => row[pivotKey] === rowData[pivotKey]).length > 0}
                                rowData={rowData}
